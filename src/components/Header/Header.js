    ...
    <div className="w-full rounded-md font-sans flex justify-between items-center bg-pink-50 p-2 shadow-md px-5 relative">
      ...
      <div className="flex-grow flex justify-center">
        <ul className="flex gap-2 items-center">
          ...
          <div className="bg-gray-200 p-2 flex gap-2 items-center rounded font-semibold text-2xl">
            {loggedInUser || "Guest"} 
            {isOnline ? (
              <div className="w-5 h-5 rounded-full bg-green-600"> </div>
            ) : (
              <div className="w-5 h-5 rounded-full bg-gray-400 border border-slate-700 shadow-lg"></div>
            )}
          </div>
        </ul>
      </div>
